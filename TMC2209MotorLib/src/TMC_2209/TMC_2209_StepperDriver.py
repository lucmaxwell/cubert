...
class TMC_2209:
    ...
    def __init__(self, pin_en=-1, pin_step=-1, pin_dir=-1, baudrate=115200, serialport="/dev/serial0",
                 driver_address=0, gpio_mode=GPIO.BCM, loglevel=None, skip_uart_init=False):
        """constructor

        Args:
            pin_en (int, optional): EN pin number. Defaults to -1.
    ...
    def set_vactual_dur_alt(self, vactual, duration=0, acceleration=0,
        ...
        vactual = tmc_math.rps_to_vactual(tmc_math.steps_to_rps(1/(duration/1000000), self._steps_per_rev), self._steps_per_rev)
