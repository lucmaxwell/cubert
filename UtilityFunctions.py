...
def load_model_PPO(model_name, num_scramble=1):
    ...
    if os.path.isfile(model_file_path):
        ...
        training_model = PPO.load(model_file_path, env=env, verbose=2, tensorboard_log=log_path, device="cuda")
    else:
        print("Creating a new model...")
        training_model = PPO('MlpPolicy', env=env, verbose=2, tensorboard_log=log_path, device="cuda")
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path=os.path.join(save_path, model_name + "_best_model"),
                                 log_path=os.path.join(log_path, model_name + "_best_model"), eval_freq=10000,
                                 deterministic=True)
...
def load_model_DQN(model_name):
    ...
    if os.path.isfile(model_file_path):
        ...
        training_model = DQN.load(model_file_path, env=env, verbose=2,tensorboard_log=log_path, device="cuda")
    else:
        print("Creating a new model...")
        training_model = DQN('MlpPolicy', env=env, verbose=2, tensorboard_log=log_path, device="cuda")
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path=os.path.join(save_path, model_name + "_best_model"),
                                 log_path=os.path.join(log_path, "_best_model"), eval_freq=500,
                                 deterministic=True)
    callback = CallbackList([checkpoint_callback, eval_callback])
