...
def load_model_PPO(model_name, neural_network_class, num_scramble):
    ...
    policy_kwargs = dict(
        features_extractor_class=neural_network_class,
    ...
    eval_callback = EvalCallback(eval_env, best_model_save_path=os.path.join(save_path, model_name + "_best_model"),
                                 log_path=os.path.join(model_log_path, model_name + "_best_model"), eval_freq=100000,
                                 deterministic=True)
...
def load_model_DQN(model_name, neural_network_class, num_scramble):
    print(torch.__version__)
    print(f"CUDA is available: {torch.cuda.is_available()}")
